...
public class CategoriaResource {
	...
	@RequestMapping(method = RequestMethod.POST)// quando eu fizer um POST /categorias/, o fluxo vem pra esse método aqui embaixo
	public ResponseEntity<?> insert(@RequestBody Categoria obj) {// @RequestBody permite pegar o corpo da requisição e passar para dentro do método
		// o <?> depois do ResponseEntity quer dizer que qualquer tipo de objeto pode ser retornado
		// o ResponseEntity permite devolver para o cliente uma resposta mais completa
		...
		// vai retornar algo assim, por exemplo: http://localhost:8080/categorias/5 (sendo 5 o id da categoria recém criada)
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.PUT)// quando eu fizer um PUT /categorias/id, o fluxo vem pra esse método aqui embaixo
	...
	@RequestMapping(value = "/{id}", method = RequestMethod.DELETE)// quando eu fizer um DELETE /categorias/id, o fluxo vem pra esse método aqui embaixo
